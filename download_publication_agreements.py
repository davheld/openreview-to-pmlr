...
if __name__ == "__main__":
    ...
    if not os.path.exists("corl2024_pub_agreements"):
        os.makedirs("corl2024_pub_agreements")

    missing_pub_agreements = []
    ...
    for submission in submissions:
        ...
        if "publication_agreement" in submission.content:
            pdf_url = "{0}{1}".format(client.baseurl, submission.content["publication_agreement"])
            paper_number = submission.number
            try:
                with open("corl2024_pub_agreements/Paper{0}.pdf".format(paper_number), "wb") as f:
                    # f.write(client.get_pdf(submission.id))
                    f.write(client.get_attachment(submission.id, "publication_agreement"))
            ...
            missing_pub_agreements.append(submission.number)

    print(f"Submissions with missing publication agreements: {len(missing_pub_agreements)}")
    print(missing_pub_agreements)
